...
- **Phoenix defaults that are changed**:
    ...
    - A `lib/tc/release.ex` module has been added to run migrations with releases
...
```sh
git clone https://github.com/nickjj/docker-phoenix-example tc
cd tc
...
The app is named `tc` right now but chances are your app will be a different
name. Since the app is already created we'll need to do a find / replace on a
few variants of the string "tc" and update a few Docker related resources.
